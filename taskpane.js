...
Office.onReady(() => {
  ...
  Office.actions.associate("FormulaExplorer.Exit", () => exitNavigation());
  const exitBtn = document.getElementById("exit");
  if (exitBtn) exitBtn.onclick = () => exitNavigation();
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") exitNavigation();
  });
});
...
async function selectionStillBelongsToSession() {
  ...
  return Excel.run(async context => {
    ...
    const activeAddress = cell.address.includes("!") ? cell.address.split("!").slice(1).join("!") : cell.address;
    return sheet.name === expected.sheet && normalizeAddress(activeAddress) === normalizeAddress(expected.address);
  });
}
